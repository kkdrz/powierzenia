  ...
  return (
    <div>
      <h2 id="course-heading">
        <Translate contentKey="powierzeniaApp.course.home.title">Courses</Translate>
        <Link to={`${match.url}/new`} className="btn btn-primary float-right jh-create-entity" id="jh-create-entity">
          <FontAwesomeIcon icon="plus" />
          &nbsp;
          <Translate contentKey="powierzeniaApp.course.home.createLabel">Create new Course</Translate>
        </Link>
      </h2>
      <div className="table-responsive">
        {courseList && courseList.length > 0 ? (
            ...
            <thead>
              <tr>
                ...
                <th className="hand" onClick={sort('name')}>
                  <Translate contentKey="powierzeniaApp.course.name">Name</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th className="hand" onClick={sort('code')}>
                  <Translate contentKey="powierzeniaApp.course.code">Code</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th>
                  <Translate contentKey="powierzeniaApp.course.educationPlan">Education Plan</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {courseList.map((course, i) => (
                <tr key={`entity-${i}`}>
                  ...
                  <td>{course.code}</td>
                  <td>
                    {course.educationPlanId ? <Link to={`education-plan/${course.educationPlanId}`}>{course.educationPlanId}</Link> : ''}
                  </td>
          ...
          <div className="alert alert-warning">
            <Translate contentKey="powierzeniaApp.course.home.notFound">No Courses found</Translate>
          </div>
        )}
      </div>
